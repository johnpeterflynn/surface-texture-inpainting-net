...
class FastInstanceNorm(_InstanceNorm):
    ...
    def __init__(self, in_channels, eps=1e-5, momentum=0.1, affine=False,
        ...
        super(FastInstanceNorm, self).__init__(in_channels, eps, momentum, affine,
                                           track_running_stats)
    ...
    def forward(self, x: Tensor, batch: OptTensor = None) -> Tensor:
        ...
        batch_ptr = torch.linspace(0, x.shape[0], batch_size + 1, dtype=torch.int, device=batch.device)

        mean = var = unbiased_var = x  # Dummies.

        if self.training or not self.track_running_stats:
            num_features = x.shape[1]
            #norm = (batch_ptr - batch_ptr.roll(1))[1:]
            ...
            mean = torch.empty((batch_size, num_features), dtype=x.dtype, device=batch.device)
            for i in range(1, len(batch_ptr)):
                mean[i-1] = x[batch_ptr[i-1]:batch_ptr[i]].sum(dim=0)
            mean /= norm

            #x = x - mean[batch]
            #x = x - mean.repeat_interleave(len(batch) // batch_size, dim=0)
            x = x - mean.index_select(0, batch)

            #var = scatter(x * x, batch, dim=0, dim_size=batch_size,
            #              reduce='add')
            var = torch.empty((batch_size, num_features), dtype=x.dtype, device=batch.device)
            for i in range(1, len(batch_ptr)):
                var[i-1] = x[batch_ptr[i-1]:batch_ptr[i]].pow(2).sum(dim=0)
        ...
        #out = x / (var + self.eps).sqrt().repeat_interleave(len(batch) // batch_size, dim=0)
