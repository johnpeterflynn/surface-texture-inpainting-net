...
class SingleBatchGraphNorm(torch.nn.Module):
    ...
    def __init__(self, in_channels: int, eps: float = 1e-5):
        super(SingleBatchGraphNorm, self).__init__()
    ...
    def forward(self, x: Tensor, batch: Optional[Tensor] = None) -> Tensor:
        ...
        for i in range(1, len(batch_ptr)):
            mean[i - 1] = x[batch_ptr[i - 1]:batch_ptr[i]].mean(dim=0)
        ...
        for i in range(1, len(batch_ptr)):
            var[i - 1] = x[batch_ptr[i - 1]:batch_ptr[i]].pow(2).mean(dim=0)
